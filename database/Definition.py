...
class TableDefinition:
    ...
    def table_definition(self):
        try:
            self.cursor.execute("CREATE TABLE course(`course_id` varchar(6) PRIMARY KEY, `course_name` varchar(50));")
            self.cursor.execute("CREATE TABLE student (`stud_id` varchar(8) PRIMARY KEY, `stud_name` TEXT, `year` INT, `email` varchar(30), `course_id` varchar(6) , FOREIGN KEY (course_id) REFERENCES course(course_id) );")
            self.cursor.execute("CREATE table users(`user_id` varchar(8) PRIMARY KEY, `role` ENUM ('module_staff','welfare_staff') NOT NULL, `email` varchar(30) );")
            self.cursor.execute("CREATE TABLE module (`mod_id` varchar(8) PRIMARY KEY, `mod_name` TEXT, `course_id` varchar(6), `year` INT, `welfare_staff_id` varchar(8), `module_staff_id` varchar(8) , FOREIGN KEY (welfare_staff_id) REFERENCES users(user_id) ,FOREIGN KEY (module_staff_id) REFERENCES users(user_id) ,FOREIGN KEY (course_id) REFERENCES course(course_id));")
