...
class TableDefinition:
    ...
    def table_definition(self):
        try:
            self.cursor.execute("CREATE TABLE course(`course_id` varchar(6) PRIMARY KEY, `course_name` varchar(50) NOT NULL);")
            self.cursor.execute("CREATE TABLE student (`stud_id` varchar(8) PRIMARY KEY, `stud_name` TEXT NOT NULL, `year` INT NOT NULL, `email` varchar(30), `course_id` varchar(6) , FOREIGN KEY (course_id) REFERENCES course(course_id) );")
            self.cursor.execute("CREATE table users(`user_id` varchar(8) PRIMARY KEY, `user_name` TEXT NOT NULL, `role` ENUM ('module_staff','welfare_staff') NOT NULL, `email` varchar(30) );")
            self.cursor.execute("CREATE TABLE module (`mod_id` varchar(8) PRIMARY KEY, `mod_name` TEXT NOT NULL, `course_id` varchar(6), `year` INT, `welfare_staff_id` varchar(8), `module_staff_id` varchar(8) , FOREIGN KEY (welfare_staff_id) REFERENCES users(user_id) ,FOREIGN KEY (module_staff_id) REFERENCES users(user_id) ,FOREIGN KEY (course_id) REFERENCES course(course_id));")
            self.cursor.execute("CREATE TABLE attendance (`week_no` INT, `mod_id` varchar(8), `stud_id` varchar(8) , `att_per` DECIMAL(5,2) , UNIQUE KEY unique_stu_att_per_week(week_no, mod_id, stud_id), FOREIGN KEY (stud_id) REFERENCES student(stud_id) , FOREIGN KEY (mod_id) REFERENCES module(mod_id));")
            ...
            self.cursor.execute("CREATE TABLE deadlines (`dead_id` INT PRIMARY KEY, `mod_id` varchar(8) , `week_no` INT, `ass_name` varchar(100) DEFAULT 'Assessment', `due_date` DATE, FOREIGN KEY (mod_id) REFERENCES module(mod_id));")
            self.conn.commit()
